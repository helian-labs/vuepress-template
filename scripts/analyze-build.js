...
async function analyze() {
  ...
  console.log(`总大小: ${formatSize(stats.totalSize)} (压缩后: ${formatSize(stats.compressedSize)})`);

  // 按类型显示
  console.log(chalk.cyan('\n文件类型分布:'));
  Object.keys(stats.fileTypes).sort().forEach(type => {
    const typeStats = stats.fileTypes[type];
    console.log(`  ${type}: ${typeStats.count}个文件, ${formatSize(typeStats.size)} (${(typeStats.size / stats.totalSize * 100).toFixed(1)}%)`);
  });
