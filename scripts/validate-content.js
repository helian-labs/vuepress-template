...
function validateLinks(content, filePath, relativeMdPath) {
  ...
  while ((match = mdLinkRegex.exec(content)) !== null) {
    ...
    // 处理相对路径
    const targetPath = linkPath.endsWith('.md') || linkPath.endsWith('.html')
...
function validateAnchorLink(content, anchor, filePath) {
  ...
  const headingRegex = new RegExp(`<h[1-6][^>]*id=["']${headingId}["'][^>]*>|^#+\\s+(.+?)\\s*$`, 'gm');
...
function validateHeadings(content, filePath) {
  ...
  headings.forEach(heading => {
    if (headingTexts[heading.text]) {
      warnings.push(`${filePath}: 重复标题 "${heading.text}" 在第 ${headingTexts[heading.text]} 行和第 ${heading.line} 行`);
    } else {
    ...
    Object.keys(SPELLING_CORRECTIONS).forEach(misspelled => {
      ...
      if (regex.test(line)) {
        ...
        if (!line.includes(correctTerm)) {
          warnings.push(`${filePath}:${lineIndex + 1}: 可能的拼写错误: "${misspelled}" 应为 "${correctTerm}"`);
        }
      }
    });
    ...
    Object.keys(CHINESE_SPELLING_CORRECTIONS).forEach(term => {
      if (line.includes(term)) {
        ...
        if (term !== suggestion && !line.includes(suggestion)) {
          warnings.push(`${filePath}:${lineIndex + 1}: 可能的用词建议: "${term}" 建议使用 "${suggestion}"`);
        }
      }
    });
  });
...
async function checkUrl(url, filePath, lineNum, isImage = false) {
  ...
  try {
    ...
    if (result.redirectTo) {
      warnings.push(`${filePath}:${lineNum}: ${isImage ? '图片' : '链接'} 重定向: ${url} -> ${result.redirectTo}`);
    }
  } catch (error) {
    // 缓存错误结果
    checkedUrls.set(url, {
    ...
    errors.push(`${filePath}:${lineNum}: ${isImage ? '图片' : '链接'} 格式无效: ${url} (${error.message})`);
  }
}
